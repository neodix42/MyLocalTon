  ...
  private static CompletableFuture<Void> doBackgroundInitialization() {
    return CompletableFuture.runAsync(() -> {
      ...
      if (Arrays.asList(appArgs).contains("test-binaries")) {
        try {
          new ValidatorEngineExecutor().execute(genesisNode, "-V").getLeft().waitFor();
        } catch (Exception e) {
          log.error("Binary test failed");
          System.exit(10);
        }
        System.out.println("Binary test passed");
        System.exit(0);
      }
      ...
      for (String nodeName : settings.getActiveNodes()) {
        if (!nodeName.contains("genesis")) {
          new ValidatorEngine().startValidator(settings.getNodeByName(nodeName),
              genesisNode.getNodeGlobalConfigLocation());
        }
      }
            ...
            if (i > 0 && i <= 6) {
              ...
              for (int j = 2; j <= i + 1; j++) {
                ...
                if (!settings.getActiveNodes().contains(node.getNodeName())) {
                  ...
                  MyLocalTon.getInstance().createWalletEntity(node, null, WalletVersion.V3R2, -1L,
  ...
  public void start(Stage stage) {
    ...
    primaryStage.setOnShown(windowEvent -> {
      ...
      if (MyLocalTon.getInstance().getSettings().getActiveNodes().isEmpty()) {
        mainController.showLoadingPane("Initializing TON blockchain very first time",
            "It can take up to 2 minutes");
      } else if (MyLocalTon.getInstance().getSettings().getActiveNodes().size() == 1) {
        mainController.showLoadingPane("Starting TON blockchain",
            "Should take no longer than 45 seconds");
      } else {
        mainController.showLoadingPane("Starting TON blockchain",
            "Launching " + MyLocalTon.getInstance().getSettings().getActiveNodes().size()
                + " validators, wait up to 3 minutes");
      }

      // Running all the heavy initialization in the background thread
      doBackgroundInitialization().thenRun(() -> {

...
import static org.ton.mylocalton.settings.MyLocalTonSettings.SETTINGS_FILE;
...
public class MyLocalTonUtils {
  public static final String WALLET_V1_CODE =
  ...
  public static final String WALLET_V2_CODE =
  ...
  public static final String WALLET_V3_CODE =
  ...
  public static final String WALLET_MASTER =
      "FF0020DD2082014C97BA9730ED44D0D70B1FE0A4F260810200D71820D70B1FED44D0D31FD3FFD15112BAF2A122F901541044F910F2A2F80001D31F31D307D4D101FB00A4C8CB1FCBFFC9ED54";
    ...
    return DateTimeFormatter.ofPattern("yyyy.MM.dd HH:mm:ss")
        .format(
            LocalDateTime.ofInstant(
                Instant.ofEpochMilli(timestamp * 1000), ZoneId.systemDefault()));
  }
    ...
    try {
      ...
      if (Main.appActive.get()) {
        ...
        if (SystemUtils.IS_OS_WINDOWS) {
          ...
          do {
            Process proc =
                Runtime.getRuntime().exec("tasklist.exe /FI \"IMAGENAME eq validator-engine.exe\"");
            ...
            if (proc.waitFor() == 0) {
              ...
              if (resultInput.contains("validator-engine")) {
                int pid =
                    Integer.parseInt(
                        StringUtils.substringBetween(resultInput, "validator-engine.exe", "Console")
                            .trim());
    ...
    try {
      ...
      if (proc.waitFor() == 0) {
        ...
        return resultInput.contains("M1")
            || resultInput.contains("M2")
            || resultInput.contains("M3")
            || resultInput.contains("M4");
      }
    } catch (Exception e) {
  ...
  public static String constructFullBlockSeq(
      Long wc, String shard, BigInteger seqno, String rootHash, String fileHash) {
  ...
  public static void replaceOutPortInConfigJson(String path, Integer port) throws IOException {
    String contentConfigJson =
        Files.readString(Paths.get(path + "config.json"), StandardCharsets.UTF_8);
    String replacedConfigJson =
        StringUtils.replace(contentConfigJson, "3278", String.valueOf(port));
    Files.writeString(
        Paths.get(path + "config.json"), replacedConfigJson, StandardOpenOption.CREATE);
  }
  ...
  public static ValidationParam getConfig(Node node) throws Exception {
    ...
    return ValidationParam.builder()
        ...
        .nextElections(
            activeElectionId
                - config15.getElectionsStartBefore()
                + config15.getValidatorsElectedFor())
  ...
  public static void participate(Node node, ValidationParam v) {
    try {
      ...
      if (node.getElectionsCounter().getOrDefault(electionId, -1L) > YEAR_1971) {
        log.info(
            "{} has already sent request ({}) for elections. Current time {}",
            node.getNodeName(),
            MyLocalTonUtils.toLocal(electionId),
            MyLocalTonUtils.toLocal(MyLocalTonUtils.getCurrentTimeSeconds()));
      ...
      if (isNull(node.getWalletAddress())) {
        log.info(
            "{} controlling smart-contract (wallet) is not present yet, cancel participation",
            node.getNodeName());
        return;
      } else {
      ...
      log.info(
          "{} with wallet {} wants to participate in elections {} ({})",
          node.getNodeName(),
      ...
      String signature =
          fift.createValidatorElectionRequest(
              node, electionId, settings.getBlockchainSettings().getMaxFactor());
      fift.signValidatorElectionRequest(
          node, electionId, settings.getBlockchainSettings().getMaxFactor(), signature);
      ...
      log.error(
          "Error by {} participating in elections! Error {}", node.getNodeName(), e.getMessage());
  ...
  public static Node getNewNode() {
    ...
    for (String n :
        Arrays.asList("genesis", "node2", "node3", "node4", "node5", "node6", "node7")) {
  ...
  public static String getNodeNameByWalletAddress(String walletAddress) {
    ...
    if (nonNull(settings.getGenesisNode().getWalletAddress())
        && walletAddress.equals(
            settings.getGenesisNode().getWalletAddress().getFullWalletAddress())) {
      return "Validator genesis, ";
    } else if (nonNull(settings.getNode2().getWalletAddress())
        && walletAddress.equals(settings.getNode2().getWalletAddress().getFullWalletAddress())) {
      return "Validator 2, ";
    } else if (nonNull(settings.getNode3().getWalletAddress())
        && walletAddress.equals(settings.getNode3().getWalletAddress().getFullWalletAddress())) {
      return "Validator 3, ";
    } else if (nonNull(settings.getNode4().getWalletAddress())
        && walletAddress.equals(settings.getNode4().getWalletAddress().getFullWalletAddress())) {
      return "Validator 4, ";
    } else if (nonNull(settings.getNode5().getWalletAddress())
        && walletAddress.equals(settings.getNode5().getWalletAddress().getFullWalletAddress())) {
      return "Validator 5, ";
    } else if (nonNull(settings.getNode6().getWalletAddress())
        && walletAddress.equals(settings.getNode6().getWalletAddress().getFullWalletAddress())) {
      return "Validator 6, ";
    } else if (nonNull(settings.getNode7().getWalletAddress())
        && walletAddress.equals(settings.getNode7().getWalletAddress().getFullWalletAddress())) {
      return "Validator 7, ";
    }
    return "";
  }
  ...
  public static void syncWithGenesis() throws IOException {
    ...
    for (String nodeName : settings.getActiveNodes()) {
      if (!nodeName.contains("genesis")) {
        ...
        // FileUtils.copyDirectory(new File(settings.getGenesisNode().getTonDbStateDir()), new
        // File(node.getTonDbStateDir()));
      }
    }
  }
  ...
  public static void deleteWalletByFullAddress(String fullAddrress) {
    ...
    if (nonNull(walletEntity.getWallet())) {
      ...
      FileUtils.deleteQuietly(
          new File(walletEntity.getWallet().getFilenameBaseLocation() + ".addr"));
      FileUtils.deleteQuietly(
          new File(walletEntity.getWallet().getFilenameBaseLocation() + "-query.boc"));
    }
  }
      ...
      if (SystemUtils.IS_OS_WINDOWS) {
        ...
        try {
          ...
          mainController.showErrorMsg(
              "python3 installation failed. Try to install it manually.", 8);
        }
        ...
        try {
          ...
          if (p.exitValue() != 0) {
            log.error(
                "cannot install python3. Try to install it manually: sudo apt install -y python3");
            mainController.showErrorMsg(
                "python3 installation failed. Try to install it manually: sudo apt install -y python3",
                8);
          } else {
        ...
        try {
          ...
          if (p.exitValue() != 0) {
            log.error(
                "cannot install python3. Try to install it manually: brew install -q python3");
            mainController.showErrorMsg(
                "python3 installation failed. Try to install it manually: brew install -q python3",
                8);
          } else {
            log.info("python3 has been installed");
            mainController.showSuccessMsg("python3 has been successfully installed", 5);
  ...
  public static void doInstallPip() {
    ...
    try {
      ...
      if (p.exitValue() != 0) {
        ...
        mainController.showErrorMsg(
            "pip installation failed. Try to install it manually: " + cmd, 8);
      } else {
    ...
    try {
      String cmd =
          SystemUtils.IS_OS_WINDOWS
              ? "cmd /c start pip3 install -U ton-http-api"
      ...
      if (p.exitValue() != 0) {
        log.error(
            "ton-http-api installation failed. Try to install it manually: pip3 install --user ton-http-api");
        mainController.showErrorMsg(
            "ton-http-api installation failed. Try to install it manually: pip3 install --user ton-http-api",
            8);
      } else {
  ...
  public void showThreads() {
    ...
    for (int i = 0; i < noThreads; i++) {
      log.info(
          "Thread No: {}, {}", i, Objects.isNull(lstThreads[i]) ? "null" : lstThreads[i].getName());
    }
  }

  public static class VisibleParagraphStyler<PS, SEG, S>
      implements Consumer<ListModification<? extends Paragraph<PS, SEG, S>>> {
    private final GenericStyledArea<PS, SEG, S> area;
    private final Function<String, StyleSpans<S>> computeStyles;
    private int prevParagraph, prevTextLength;

    public VisibleParagraphStyler(

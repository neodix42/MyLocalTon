...
import org.ton.mylocalton.executors.createstate.CreateStateExecutor;
import org.ton.mylocalton.executors.generaterandomid.GenerateRandomId;
import org.ton.mylocalton.executors.generaterandomid.RandomIdExecutor;
...
public class ValidatorEngine {
  ...
  public Process startValidator(Node node, String myGlobalConfig) {
    ...
    Pair<Process, Future<String>> validator =
        new ValidatorEngineExecutor()
            .execute(
                node,
                "--verbosity",
                MyLocalTonUtils.getTonLogLevel(node.getTonLogLevel()),
                "--threads",
                "4",
                "--global-config",
                myGlobalConfig,
                ...
                node.getTonDbDir(),
                "--logname",
                node.getTonLogDir() + MyLocalTonUtils.toUtcNoSpace(System.currentTimeMillis()),
                "--session-logs",
                "",
                "--ip",
                node.getPublicIp() + ":" + node.getPublicPort(),
                "--sync-before",
                node.getValidatorSyncBefore()
                    .toString(), // initial sync download all blocks for last given seconds, default
                // 3600
                "--state-ttl",
                node.getValidatorStateTtl()
                    .toString(), // state will be gc'd after this time (in seconds), default 3600
                "--block-ttl",
                node.getValidatorBlockTtl()
                    .toString(), // blocks will be gc'd after this time (in seconds),
                // default=7*86400
                "--archive-ttl",
                node.getValidatorArchiveTtl()
                    .toString(), // archived blocks will be deleted after this time (in seconds),
                // default 365*86400
                "--key-proof-ttl",
                node.getValidatorKeyProofTtl()
    ...
    log.info(
        "{} validator-engine started at {}",
        node.getNodeName(),
        node.getPublicIp() + ":" + node.getPublicPort());
    return validator.getLeft();
  }

  public Pair<Process, Future<String>> startValidatorWithoutParams(
      Node node, String myGlobalConfig) {
  ...
  public void initFullnode(Node node, String sharedGlobalConfig) throws Exception {
    if (Files.exists(Paths.get(node.getTonDbDir() + "state"))) {
      ...
      Files.copy(
          Paths.get(sharedGlobalConfig),
          Paths.get(node.getNodeGlobalConfigLocation()),
      ...
      // enable access to full node from validator-engine-console - required if you want to become
      // validator later
  ...
  public void enableLiteServer(Node node, String myGlobalConfig, boolean reinstall)
      throws Exception {
    ...
    if (Files.exists(Paths.get(node.getTonDbKeyringDir() + "liteserver")) && (!reinstall)) {
      ...
      String liteServers =
          "\"liteservers\" : [{\"id\":\""
              + liteServerKeys.getRight()
              + "\",\"port\":\""
              + node.getLiteServerPort()
              + "\"}";
      log.debug("liteservers: {} ", liteServers);

      // convert pub key to key
      String liteserverPubkeyBase64 =
          MyLocalTonUtils.convertPubKeyToBase64(node.getTonDbKeyringDir() + "liteserver.pub");
      int publicIpNum = MyLocalTonUtils.getIntegerIp(node.getPublicIp());

      // replace lite servers array in config.json
      String configJson =
          FileUtils.readFileToString(
              new File(node.getTonDbDir() + "config.json"), StandardCharsets.UTF_8);
      String existingLiteservers =
          "\"liteservers\" : " + MyLocalTonUtils.sbb(configJson, "\"liteservers\" : [");
      String configJsonNew =
          StringUtils.replace(configJson, existingLiteservers, liteServers + "\n]");
      FileUtils.writeStringToFile(
          new File(node.getTonDbDir() + "config.json"), configJsonNew, StandardCharsets.UTF_8);
      // done with config.json

      String myGlobalTonConfig =
          FileUtils.readFileToString(new File(myGlobalConfig), StandardCharsets.UTF_8);
      ...
      if (myGlobalTonConfig.contains("liteservers")) {
        ...
        liteServerConfigNew =
            "{\"id\":{\"key\":\""
                + liteserverPubkeyBase64
                + "\", \"@type\":\"pub.ed25519\"}, \"port\": "
                + node.getLiteServerPort()
                + ", \"ip\": "
                + publicIpNum
                + "}\n]";
        ...
        myGlobalTonConfigNew =
            StringUtils.replace(myGlobalTonConfig, existingLiteserver, "[" + liteServerConfigNew);
        FileUtils.writeStringToFile(
            new File(node.getNodeLocalConfigLocation()),
            myGlobalTonConfigNew,
        ...
        liteServerConfigNew =
            "\"liteservers\":[{\"id\":{\"key\":\""
                ...
                + "}\n],\n \"validator\": {";
        myGlobalTonConfigNew =
            StringUtils.replace(myGlobalTonConfig, "\"validator\": {", liteServerConfigNew);
        FileUtils.writeStringToFile(
            new File(myGlobalConfig), myGlobalTonConfigNew, StandardCharsets.UTF_8);
        FileUtils.writeStringToFile(
            new File(node.getNodeLocalConfigLocation()),
            myGlobalTonConfigNew,
            StandardCharsets.UTF_8);
  ...
  public void createZeroState(Node node) throws Exception {
    ...
    byte[] zerostateRootHashFile =
        Files.readAllBytes(
            Paths.get(node.getTonBinDir() + ZEROSTATE + File.separator + "zerostate.rhash"));
    byte[] zerostateFileHashFile =
        Files.readAllBytes(
            Paths.get(node.getTonBinDir() + ZEROSTATE + File.separator + "zerostate.fhash"));
    ...
    Files.move(
        Paths.get(node.getTonBinDir() + ZEROSTATE + File.separator + "zerostate.boc"),
        Paths.get(node.getTonDbStaticDir() + settings.getZeroStateFileHashHex()),
        StandardCopyOption.REPLACE_EXISTING);

    // mv basestate0.boc ../db/static/$BASESTATE_FILEHASH
    Files.move(
        Paths.get(node.getTonBinDir() + ZEROSTATE + File.separator + "basestate0.boc"),
        Paths.get(node.getTonDbStaticDir() + settings.getBaseStateFileHashHex()),
        StandardCopyOption.REPLACE_EXISTING);

    String content =
        Files.readString(
            Paths.get(
                Extractor.MY_LOCAL_TON_ROOT_DIR
                    + TEMPLATES
                    + File.separator
    ...
    Files.writeString(
        Paths.get(node.getNodeGlobalConfigLocation()), content, StandardCharsets.UTF_8);
  ...
  private boolean generateZeroState(Node node) throws IOException {
    ...
    String createStateResult =
        new CreateStateExecutor()
            .execute(
                node, node.getTonBinDir() + "smartcont" + File.separator + "gen-zerostate.fif");
    ...
    byte[] mainWalletPrvKey =
        FileUtils.readFileToByteArray(
            new File(node.getTonBinDir() + ZEROSTATE + File.separator + "main-wallet.pk"));
    settings.setMainWalletPrvKey(Hex.encodeHexString(mainWalletPrvKey));
    settings.setMainWalletFilenameBaseLocation(
        node.getTonBinDir() + ZEROSTATE + File.separator + "main-wallet");
    ...
    //                .filenameBaseLocation(node.getTonBinDir() + ZEROSTATE + File.separator +
    // "main-wallet")
    //                .privateKeyLocation(node.getTonBinDir() + ZEROSTATE + File.separator +
    // "main-wallet.pk")
  ...
  public void configureGenesisZeroState() throws IOException {
    ...
    String genZeroStateFifPath =
        settings.getGenesisNode().getTonBinDir()
            + File.separator
            + "smartcont"
            + File.separator
            + "gen-zerostate.fif";
    String genZeroStateFif =
        FileUtils.readFileToString(new File(genZeroStateFifPath), StandardCharsets.UTF_8);
    ...
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFif,
            "GLOBAL_ID",
            settings.getBlockchainSettings().getGlobalId().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "INITIAL_BALANCE",
            settings.getBlockchainSettings().getInitialBalance().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "GAS_PRICE_MC",
            settings.getBlockchainSettings().getGasPriceMc().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "GAS_PRICE",
            settings.getBlockchainSettings().getGasPrice().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "CELL_PRICE_MC",
            settings.getBlockchainSettings().getCellPriceMc().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "CELL_PRICE",
            settings.getBlockchainSettings().getCellPrice().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "MAX_VALIDATORS",
            settings.getBlockchainSettings().getMaxValidators().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "MAX_MAIN_VALIDATORS",
    ...
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "MIN_VALIDATORS",
            settings.getBlockchainSettings().getMinValidators().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "MIN_STAKE",
            settings.getBlockchainSettings().getMinValidatorStake().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "MAX_STAKE",
            settings.getBlockchainSettings().getMaxValidatorStake().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "MIN_TOTAL_STAKE",
    ...
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "MAX_FACTOR",
            settings.getBlockchainSettings().getMaxFactor().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
            "ELECTED_FOR",
            settings.getBlockchainSettings().getElectedFor().toString());
    genZeroStateFifNew =
        StringUtils.replace(
            genZeroStateFifNew,
  ...
  public void generateValidatorKeys(Node node, boolean updateGenZeroStateFif) throws Exception {
    ...
    String validatorKeys =
        new RandomIdExecutor()
            .execute(node, "-m", "keys", "-n", node.getTonDbKeyringDir() + VALIDATOR);
    ...
    log.debug(
        "{}, validatorPrvKeyHex {}, validatorPrvKeyBase64 {}",
        node.getNodeName(),
        validatorPrvKeyHex,
        validatorPrvKeyBase64);

    Files.copy(
        Paths.get(node.getTonDbKeyringDir() + VALIDATOR),
        Paths.get(node.getTonDbKeyringDir() + validatorPrvKeyHex),
        StandardCopyOption.REPLACE_EXISTING);
    // convert pub key to key
    byte[] validatorPubKey =
        Files.readAllBytes(Paths.get(node.getTonDbKeyringDir() + "validator.pub"));
    ...
    Files.write(
        Paths.get(node.getValidatorKeyPubLocation()),
        Hex.decodeHex(node.getValidatorPubKeyHex()),
        StandardOpenOption.CREATE); // "validator-keys-1.pub"

    if (updateGenZeroStateFif) {
      // replace path to validator-key-1.pub in gen-zerostate.fif
      String genZeroStateFif =
          FileUtils.readFileToString(
              new File(node.getGenesisGenZeroStateFifLocation()), StandardCharsets.UTF_8);
      String genZeroStateFifNew =
          StringUtils.replace(
              genZeroStateFif,
              "// \"path_to_" + node.getNodeName() + "_pub_key\"",
              "\"" + node.getValidatorKeyPubLocation() + "\"");
      genZeroStateFifNew =
          StringUtils.replace(
              genZeroStateFifNew,
              "initial_stake_" + node.getNodeName(),
              node.getDefaultValidatorStake().min(BigInteger.ONE).toString());
      FileUtils.writeStringToFile(
          new File(node.getGenesisGenZeroStateFifLocation()),
          genZeroStateFifNew,
          StandardCharsets.UTF_8);
    }
  }

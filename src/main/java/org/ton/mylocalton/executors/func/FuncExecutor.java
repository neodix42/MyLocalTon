package org.ton.mylocalton.executors.func;
  ...
  private void extractFunc() throws IOException {
    synchronized (FuncExecutor.class) {
      if (SystemUtils.IS_OS_WINDOWS) {
        if (Files.notExists(
            Paths.get(installDir + File.separator + FUNC_EXE), LinkOption.NOFOLLOW_LINKS)) {
          log.info("extracting {} on Windows", FUNC_EXE);
          Files.createDirectories(Paths.get(installDir));
          InputStream windowsFunc =
              FuncExecutor.class
                  .getClassLoader()
                  .getResourceAsStream(
                      "org/ton/mylocalton/binaries/" + FUNC + "/win64/" + FUNC_EXE); // TODO
          Files.copy(
              windowsFunc,
              Paths.get(installDir + File.separator + FUNC_EXE),
              StandardCopyOption.REPLACE_EXISTING);
          windowsFunc.close();
          log.info("windows {} path: {}", FUNC_EXE, installDir + File.separator + FUNC_EXE);
        } else {
          log.info(
              "{} on Windows already extracted into {}, use it.",
              FUNC_EXE,
              installDir + File.separator + FUNC_EXE);
        }
      } else if (SystemUtils.IS_OS_LINUX) {
        if (Files.notExists(
            Paths.get(installDir + File.separator + FUNC), LinkOption.NOFOLLOW_LINKS)) {
          log.info("extracting {} on Unix", FUNC);
          Files.createDirectories(Paths.get(installDir));
          InputStream ubuntuFunc =
              FuncExecutor.class
                  .getClassLoader()
                  .getResourceAsStream("org/ton/mylocalton/binaries/" + FUNC + "/ubuntu/" + FUNC);
          Files.copy(
              ubuntuFunc,
              Paths.get(installDir + File.separator + FUNC),
              StandardCopyOption.REPLACE_EXISTING);
          ubuntuFunc.close();
          ...
          log.info(
              "{} on Unix already extracted into {}, use it.",
              FUNC,
              installDir + File.separator + FUNC);
        }
      } else {
        log.error(
            "You are running neither on Windows nor Unix. We don't have compiled client for it.");
      }
    }
  }

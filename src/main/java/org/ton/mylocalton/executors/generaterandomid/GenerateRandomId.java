package org.ton.mylocalton.executors.generaterandomid;
...
public class GenerateRandomId {
  ...
  /**
   * Creates server key in db/ and puts into db/keyring/hex. Replaces CONSOLE-PORT, SERVER-ID,
   * CLIENT-ID in control.template and puts them into db/config.json
   */
  public void generateClientCertificate(Node node, String serverIdHuman) throws Exception {
    ...
    if (Files.exists(Paths.get(node.getTonBinDir() + "certs" + File.separator + "client"))) {
      ...
      String replacedTemplate =
          StringUtils.replace(content, "CONSOLE-PORT", String.valueOf(node.getConsolePort()));
      replacedTemplate =
          StringUtils.replace(replacedTemplate, "SERVER-ID", "\"" + serverIdHuman + "\"");
      replacedTemplate =
          StringUtils.replace(replacedTemplate, "CLIENT-ID", "\"" + clientIdBase64 + "\"");

      String configFile =
          Files.readString(Paths.get(node.getTonDbDir() + "config.json"), StandardCharsets.UTF_8);
      String configNew = StringUtils.replace(configFile, "\"control\" : [", replacedTemplate);
      Files.writeString(
          Paths.get(node.getTonDbDir() + "config.json"), configNew, StandardOpenOption.CREATE);
    }
  }
  ...
  public void getClientCertificate(Node node, String serverIdHuman) throws Exception {
    ...
    if (Files.exists(Paths.get(node.getTonBinDir() + "certs" + File.separator + "client"))) {
      ...
      InputStream stream =
          Extractor.class.getClassLoader().getResourceAsStream("org/ton/mylocalton/certs/client");
      Files.copy(
          stream,
          Paths.get(node.getTonBinDir() + "certs" + File.separator + "client"),
      ...
      stream =
          Extractor.class
              .getClassLoader()
              .getResourceAsStream("org/ton/mylocalton/certs/client.pub");
      Files.copy(
          stream,
          Paths.get(node.getTonBinDir() + "certs" + File.separator + "client.pub"),
          StandardCopyOption.REPLACE_EXISTING);
      stream.close();
  ...
  public String generateServerCertificate(Node node) throws Exception {
    ...
    if (Files.exists(Paths.get(node.getTonBinDir() + "certs" + File.separator + "server"))) {
      ...
      String serverIds =
          new RandomIdExecutor()
              .execute(
                  ...
                  "keys",
                  "-n",
                  node.getTonBinDir() + "certs" + File.separator + "server");
      ...
      Files.copy(
          Paths.get(node.getTonBinDir() + "certs" + File.separator + "server"),
          Paths.get(node.getTonDbKeyringDir() + serverIdHex),
          StandardCopyOption.REPLACE_EXISTING);
  ...
  public String getServerCertificate(Node node) throws Exception {
    ...
    if (Files.exists(Paths.get(node.getTonBinDir() + "certs" + File.separator + "server"))) {
      ...
      InputStream stream =
          Extractor.class.getClassLoader().getResourceAsStream("org/ton/mylocalton/certs/server");
      Files.copy(
          stream,
          Paths.get(node.getTonDbKeyringDir() + serverIdHex),
          StandardCopyOption.REPLACE_EXISTING);
      ...
      stream =
          Extractor.class
              .getClassLoader()
              .getResourceAsStream("org/ton/mylocalton/certs/server.pub");
      Files.copy(
          stream,
          Paths.get(node.getTonBinDir() + "certs" + File.separator + "server.pub"),
          StandardCopyOption.REPLACE_EXISTING);
      stream.close();
  ...
  public Pair<String, String> generateLiteServerKeys(Node node) throws Exception {
    String liteserverKeys =
        new RandomIdExecutor()
            .execute(node, "-m", "keys", "-n", node.getTonDbKeyringDir() + "liteserver");
    ...
    log.debug(
        "liteServerIdHex {}, liteServerIdBase64 {} on {}",
        liteServerIdHex,
        liteServerIdBase64,
        node.getNodeName());

    Files.copy(
        Paths.get(node.getTonDbKeyringDir() + "liteserver"),
        Paths.get(node.getTonDbKeyringDir() + liteServerIdHex),
        StandardCopyOption.REPLACE_EXISTING);
  ...
  public Pair<String, String> getLiteServerKeys(Node node) throws Exception {
    ...
    InputStream stream =
        Extractor.class.getClassLoader().getResourceAsStream("org/ton/mylocalton/certs/liteserver");
    Files.copy(
        stream,
        Paths.get(node.getTonDbKeyringDir() + liteServerIdHex),
        StandardCopyOption.REPLACE_EXISTING);
    ...
    stream =
        Extractor.class
            .getClassLoader()
            .getResourceAsStream("org/ton/mylocalton/certs/liteserver.pub");
    Files.copy(
        stream,
        Paths.get(node.getTonDbKeyringDir() + "liteserver.pub"),
        StandardCopyOption.REPLACE_EXISTING);
    stream.close();

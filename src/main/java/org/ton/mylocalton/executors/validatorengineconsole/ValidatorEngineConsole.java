package org.ton.mylocalton.executors.validatorengineconsole;
...
public class ValidatorEngineConsole {
  ...
  public void getStats(Node node) throws ExecutionException, InterruptedException {
    ...
    Pair<Process, Future<String>> result =
        validatorConsole.execute(
            node,
            "-k",
            node.getTonBinDir() + "certs" + File.separator + "client",
            "-p",
            node.getTonBinDir() + "certs" + File.separator + "server.pub",
            "-v",
            "0",
            "-a",
  ...
  public void importF(Node node, String validatorIdHex)
    ...
    Pair<Process, Future<String>> result =
        validatorConsole.execute(
            ...
            node.getPublicIp() + ":" + node.getConsolePort().toString(),
            "-rc",
            "importf " + node.getTonDbKeyringDir() + validatorIdHex);
    log.debug(result.getRight().get());
    if (!result.getRight().get().contains("conn ready")) {
      throw new Error("validator-engine-console on " + node.getNodeName() + " cannot importf");
    }
  ...
  public void changeFullNodeAddr(Node node, String newNodeKey)
    ...
    Pair<Process, Future<String>> result =
        validatorConsole.execute(
            ...
            node.getPublicIp() + ":" + node.getConsolePort().toString(),
            "-rc",
            "changefullnodeaddr " + newNodeKey);
    log.debug(result.getRight().get());
    if (!result.getRight().get().contains("conn ready")) {
      throw new Error(
          "validator-engine-console on " + node.getNodeName() + " cannot changefullnodeaddr");
    }
  }

  public void addValidatorAddr(
      Node node, String validatorIdHex, String newValAdnl, long electionEnd)
      throws ExecutionException, InterruptedException {
    ...
    Pair<Process, Future<String>> result =
        validatorConsole.execute(
            ...
            node.getPublicIp() + ":" + node.getConsolePort().toString(),
            "-rc",
            "addvalidatoraddr  " + validatorIdHex + " " + newValAdnl + " " + electionEnd);
    log.debug(result.getRight().get());
    if (!result.getRight().get().contains("conn ready")) {
      throw new Error(
          "validator-engine-console on " + node.getNodeName() + " cannot addvalidatoraddr");
    }
  ...
  public void addPermKey(Node node, String validatorIdHex, long electionId, long electionEnd)
    ...
    Pair<Process, Future<String>> result =
        validatorConsole.execute(
            ...
            node.getPublicIp() + ":" + node.getConsolePort().toString(),
            "-rc",
            "addpermkey " + validatorIdHex + " " + electionId + " " + electionEnd);
    log.debug(result.getRight().get());
    if (!result.getRight().get().contains("conn ready")) {
  ...
  public void addTempKey(Node node, String validatorIdHex, long electionEnd)
    ...
    Pair<Process, Future<String>> result =
        validatorConsole.execute(
            ...
            node.getPublicIp() + ":" + node.getConsolePort().toString(),
            "-rc",
            "addtempkey " + validatorIdHex + " " + validatorIdHex + " " + electionEnd);
    log.debug(result.getRight().get());
    if (!result.getRight().get().contains("conn ready")) {
  ...
  public String generateNewNodeKey(Node node) throws ExecutionException, InterruptedException {
    ...
    if (!result.getRight().get().contains("created new key")) {
      throw new Error(
          "validator-engine-console on " + node.getNodeName() + " cannot create new key");
    }
  ...
  public String exportPubKey(Node node, String newkey)
    ...
    return result
        .getRight()
        .get()
        .substring(result.getRight().get().indexOf("got public key:") + 15)
        .trim();
  }
}

...
public class ValidatorEngineConsoleExecutor {
  ...
  public Pair<Process, Future<String>> execute(Node node, String... command) {
    ...
    String binaryPath =
        node.getTonBinDir()
            + (SystemUtils.IS_OS_WINDOWS ? VALIDATOR_ENGINE_CONSOLE_EXE : VALIDATOR_ENGINE_CONSOLE);
                  ...
                  () -> {
                    try {
                      Thread.currentThread()
                          .setName("validator-engine-console " + node.getNodeName());

                      String resultInput =
                          IOUtils.toString(p.getInputStream(), Charset.defaultCharset());
                      //                    log.debug("{} stopped", "validator-engine-console " +
                      // node.getNodeName());

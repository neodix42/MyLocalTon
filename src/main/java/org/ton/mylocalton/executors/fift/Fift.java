...
public class Fift {
  ...
  public WalletEntity getWalletByBasename(Node node, String fileBaseName)
      throws ExecutionException, InterruptedException, IOException {
    Pair<Process, Future<String>> result =
        new FiftExecutor()
            .execute(node, "smartcont" + File.separator + "show-addr.fif", fileBaseName);

    String resultStr = result.getRight().get();
    log.debug(resultStr);

    String fullAddress =
        StringUtils.substringBetween(resultStr, "Source wallet address =", EOL)
            .trim()
            .toUpperCase();

    String nonBounceableBase64url =
        StringUtils.substringBetween(
                resultStr, "Non-bounceable address, Base64Url (for init):", EOL)
            .trim();
    String bounceableBase64url =
        StringUtils.substringBetween(
                resultStr, "Bounceable address, Base64Url (for later access):", EOL)
            .trim();
    String nonBounceableBase64 =
        StringUtils.substringBetween(resultStr, "Non-bounceable address, Base64 (for init):", EOL)
            .trim();
    String bounceableBase64 =
        StringUtils.substringBetween(
                resultStr, "Bounceable address, Base64 (for later access):", EOL)
            .trim();

    String publicKey =
        StringUtils.substringBetween(resultStr, "Corresponding public key is", EOL).trim();
  ...
  /**
   * Creating a request to participate in validator elections at time startElectionTime from smart
   * contract walletAddress.
   */
  public String createValidatorElectionRequest(
      Node node, long startElectionTime, BigDecimal maxFactor)
      throws ExecutionException, InterruptedException {
    ...
    Pair<Process, Future<String>> result =
        new FiftExecutor()
            .execute(
                node,
                "smartcont" + File.separator + "validator-elect-req.fif",
    ...
    Pair<Process, Future<String>> signed =
        new ValidatorEngineConsoleExecutor()
            .execute(
                node,
                "-k",
                node.getTonCertsDir() + "client",
                "-p",
                node.getTonCertsDir() + "server.pub",
                "-v",
                "0",
                "-a",
                node.getPublicIp() + ":" + node.getConsolePort().toString(),
                "-rc",
    ...
    String signature =
        StringUtils.substring(
                signed.getRight().get(), signed.getRight().get().indexOf("signature") + 9)
            .trim();
  ...
  public void signValidatorElectionRequest(
      Node node, long startElectionTime, BigDecimal maxFactor, String signatureFromElectionRequest)
      throws ExecutionException, InterruptedException {
    ...
    Pair<Process, Future<String>> result =
        new FiftExecutor()
            .execute(
                node,
                "smartcont" + File.separator + "validator-elect-signed.fif",
                node.getWalletAddress().getBounceableAddressBase64url(),
                String.valueOf(startElectionTime),
                maxFactor.toPlainString(),
    ...
    String validatorPublicKeyHex =
        StringUtils.substringBetween(resultStr, "with validator public key ", SPACE).trim();
    BigInteger bigInt = new BigInteger(validatorPublicKeyHex, 16);
    log.debug(
        "{} signed adnl {} with validator pubkey (hex){}, (integer){}",
        node.getNodeName(),
        node.getValidationAndlKey(),
  ...
  public void createRecoverStake(Node node) throws ExecutionException, InterruptedException {
    ...
    Pair<Process, Future<String>> result =
        new FiftExecutor().execute(node, "smartcont" + File.separator + "recover-stake.fif");

package org.ton.mylocalton.executors.blockchainexplorer;
...
public class BlockchainExplorer {
  ...
  public Process startBlockchainExplorer(Node node, String globalConfigFile, int port) {
    Pair<Process, Future<String>> blockchainExplorerExecutor =
        new BlockchainExplorerExecutor()
            .execute(
                node,
                "-v",
    ...
    log.info(
        "{} native blockchain-explorer started at {}",
        node.getNodeName(),
        node.getPublicIp() + ":" + port);

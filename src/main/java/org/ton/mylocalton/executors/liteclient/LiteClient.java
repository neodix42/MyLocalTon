...
public class LiteClient {
  ...
  public String getLastCommand(Node node) {
    ...
    String binaryPath =
        node.getTonBinDir() + (SystemUtils.IS_OS_WINDOWS ? LITE_CLIENT_EXE : LITE_CLIENT);

    String[] withBinaryCommand;
    switch (config) {
      case GLOBAL:
        withBinaryCommand =
            new String[] {binaryPath, "-t", "10", "-C", node.getNodeGlobalConfigLocation(), "-c"};
        break;
      case LOCAL:
        withBinaryCommand =
            new String[] {binaryPath, "-t", "10", "-C", node.getNodeLocalConfigLocation(), "-c"};
        break;
      case FORKED:
        withBinaryCommand =
            new String[] {
              binaryPath, "-t", "10", "-C", node.getNodeForkedGlobalConfigLocation(), "-c"
            };
        break;
      default:
        withBinaryCommand =
  ...
  public String executeLast(Node node) {
    ...
    Pair<Process, Future<String>> result =
        LiteClientExecutor.getInstance(config).execute(node, command);
    if (nonNull(result)) {
      try {
  ...
  public long executeGetSeqno(Node node, String contractAddress) {
    try {
      return LiteClientParser.parseRunMethodSeqno(
          executeRunMethod(node, contractAddress, "seqno", ""));
    } catch (Exception e) {
      return -1L;
    }
  }
  ...
  public long executeGetSubWalletId(Node node, String contractAddress) {
    try {
      return LiteClientParser.parseRunMethodSeqno(
          executeRunMethod(node, contractAddress, "get_subwallet_id", ""));
    } catch (Exception e) {
      return -1L;
    }
  }
   ...
   */
  public String executeBySeqno(Node node, long wc, String shard, BigInteger seqno)
      throws Exception {
  ...
  public String executeListblocktrans(
      Node node, final ResultLastBlock resultLastBlock, final long amountOfTransactions) {
  ...
  public String executeDumptrans(
      Node node, final ResultLastBlock resultLastBlock, final ResultListBlockTransactions tx) {
    final String command =
        String.format(
            "dumptrans %s %d:%s %d",
            resultLastBlock.getFullBlockSeqno(),
  ...
  public String executeAllshards(Node node, final ResultLastBlock resultLastBlock)
      throws Exception {
  ...
  public long executeGetActiveElectionId(Node node, String electorAddr) throws Exception {
    return LiteClientParser.parseRunMethodSeqno(
        executeRunMethod(node, electorAddr, "active_election_id", ""));
  }
  ...
  public String executeComputeReturnedStake(
      Node node, String electorAddr, String validatorWalletAddr) throws Exception {
    ...
    return executeRunMethod(
        ...
        "0x" + validatorWalletAddr.trim().toLowerCase());
  }
  ...
  public List<ResultLastBlock> getShardsFromBlock(Node node, ResultLastBlock lastBlock) {
    try {
      List<ResultLastBlock> foundShardsInBlock =
          LiteClientParser.parseAllShards(executeAllshards(node, lastBlock));

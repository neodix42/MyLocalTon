...
import org.ton.mylocalton.ui.custom.control.*;
...
import org.ton.mylocalton.ui.custom.events.event.CustomRefreshEvent;
...
import org.ton.mylocalton.ui.custom.toastr.Notification;
import org.ton.mylocalton.ui.custom.toastr.NotificationAnchorStack;

public class CustomMainLayout extends AnchorPane implements Initializable {

  @FXML
  private CustomMenuButton blocksBtn,
      transactionsBtn,
      validationBtn,
      accountsBtn,
      explorerBtn,
  ...
  @FXML
  private CustomSubMenuButton logsBtn,
      foundBlocksBtn,
      foundTxsBtn,
      foundAccountsBtn,
      foundAccountsTxsBtn;
  ...
  private AnchorPane logoPane,
      ...
      foundAccountsPane,
      foundTxsPane,
      foundAccountsTxsPane;
  @FXML private HBox topButton, scrollButton, closeButton;

  @FXML private CustomSearchBar searchBar;

  @FXML private NotificationAnchorStack notificationStack;
                ...
                if (arg1.doubleValue() > 0.0) {
                  if (hasResult()) {
                    TranslateTransition tr =
                        new TranslateTransition(Duration.seconds(0.001), resultsBtn);
  ...
  private void clickExpandSettings(MouseEvent e) {
    resetButtons();
    removeCreateButton();
    if (resultsBtn.isOpened()) resultsBtn.rotateIcon();
  ...
  private void clickExpandResults(MouseEvent e) {
    resetButtons();
    removeCreateButton();
    if (settingBtn.isOpened()) settingBtn.rotateIcon();
  ...
  private void clickBlocks(Event e) {
    resetButtons();
    blocksBtn.activate();
    if (hasBlocks) changeContent(blocksPane);
    else checkBlocks();
  }
  ...
  private void clickTransactions(Event e) {
    ...
    if (hasTransactions) changeContent(transactionsPane);
    else checkTransactions();
  }
        ...
        FXMLLoader loader =
            new FXMLLoader(
                App.class
                    .getClassLoader()
                    .getResource(
                        "org/ton/mylocalton/ui/custom/media/no-blocks-transactions-pane.fxml"));
  ...
  private void setNumFoundAccountsTxs(int num, String accountAddr) {
    if (foundAccountsTxsBtn != null) resultsBtn.removeButton(foundAccountsTxsBtn);
    try {
    ...
    ((Label) foundAccountsTxsPane.lookup("#labelAccountsTxs"))
        .setText("Account " + accountAddr + " - TXs ( " + num + " )");
    if (settingBtn.isOpened()) {
      settingBtn.rotateIcon();
      settingBtn.deactivate();
    }
  ...
  private void addResultsButton() {
    ...
    if (!buttons.getChildren().contains(resultsBtn)) buttons.getChildren().add(resultsBtn);
    removeCreateButton();
    resetButtons();
    resultsBtn.activate();
    if (settingBtn.isOpened()) settingBtn.rotateIcon();
  ...
  public void handle(CustomEvent event) {
    switch (event.getEventType()) {
      ...
      case ERROR:
        ...
        break;
      case START:
        if (contentPane.getChildren().contains(blankPane)) initialState();
        break;
      ...
      case SEARCH_SIZE_ACCOUNTS_TXS:
        setNumFoundAccountsTxs(
            ((CustomSearchEvent) event).getSize(), ((CustomSearchEvent) event).getAccountAddr());
        break;
      case SEARCH_SHOW:
        addResultsButton();
        break;
  ...
  private void walletsReady() {
    if (noBlocksTransactions != null) {
      if (contentPane.getChildren().contains(noBlocksTransactions)) {
        if (noBlocksTransactionsController.getViewType()
            == NoBlocksTransactionsController.ViewType.ACCOUNTS) {
          changeContent(accountsPane);
        }
      }
      noBlocksTransactions = null;

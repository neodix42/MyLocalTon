...
public class NotificationAnchorStack extends AnchorPane {
  ...
  public String buildNotificationStackStyle() {
    return "-fx-pref-width: 200px;" + "-fx-padding: 10px;";
  }
  ...
  public void notify(Notification notification) {
    notify(notification, defFadeIn.get(), defShow.get(), defFadeOut.get(), defCollapse.get());
  }

  public void notify(Notification note, int show) {
    notify(note, defFadeIn.get(), show, defFadeOut.get(), defCollapse.get());
  }
  ...
  public void notify(Notification note, int in, int show, int out, int col) {
    ...
    transition = new FadeTransition(Duration.millis(in), note);
  ...
  private TimerTask buildFadeOut(Notification note, int out, int col) {
    return new TimerTask() {
      ...
      public void run() {
        ...
        trans = new TranslateTransition(Duration.millis(col), notificationStack);
        ...
        fade = new FadeTransition(Duration.millis(out), note);
        fade.setByValue(-1);

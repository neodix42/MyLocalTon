      ...
      keyEvent -> {
        if (!((TextField) keyEvent.getSource()).getText().matches("[\\d\\-]+")) {
          ((TextField) keyEvent.getSource())
              .setText(((TextField) keyEvent.getSource()).getText().replaceAll("[^\\d\\-]", ""));
        }
      };
  @FXML private JFXTextField workchain, subWalletID;
  @FXML private JFXButton createBtn;
  @FXML private AnchorPane anchorPane;
  @FXML private JFXComboBox<String> newWalletVersion;
        ...
        .submit(
            () -> {
              ...
              try {
                long chain =
                    Long.parseLong(
                        StringUtils.isEmpty(workchain.getText())
                            ? String.valueOf(
                ...
                long walletId =
                    Long.parseLong(
                        StringUtils.isEmpty(subWalletID.getText())
                            ? String.valueOf(
                                MyLocalTon.getInstance()
                                    .getSettings()
                                    .getWalletSettings()
                                    .getDefaultSubWalletId())
                            : subWalletID.getText());
                WalletVersion walletVersion =
                    WalletVersion.getKeyByValue(newWalletVersion.getValue());

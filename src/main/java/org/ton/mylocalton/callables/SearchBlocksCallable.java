  ...
  public BlockCallbackParam call() {
    ...
    try {
      ...
      if ((wcShardSeqnoHash.charAt(0) == '(')
        ...
        query =
            em.createQuery(
                "SELECT b FROM BlockEntity b where (b.seqno = :seqno) AND (b.shard = :shard) AND (b.wc = :wc) ORDER BY b.createdAt DESC",
                BlockEntity.class);
        results =
            query
        ...
        query =
            em.createQuery(
                "SELECT b FROM BlockEntity b where (b.roothash = :hash) OR (b.filehash = :hash) ORDER BY b.createdAt DESC",
                BlockEntity.class);
        results =
            query
        ...
        query =
            em.createQuery(
                "SELECT b FROM BlockEntity b where (b.seqno = :seqno) OR (b.shard = :shard) OR (b.wc = :wc) ORDER BY b.createdAt DESC",
                BlockEntity.class);
        results =
            query
                .setParameter(SEQNO, seqno)
